...
/obj/machinery/implantchair
	...
	attackby(var/obj/item/weapon/G as obj, var/mob/user as mob)
		if(istype(G, /obj/item/grab))
			...
			if(!ismob(grab.victim))
				return
			for(var/mob/living/carbon/slime/M in range(1,G:affecting))//affecting))//victim))
				if(M.Victim == grab.victim)
					to_chat(usr, "[grab.victim:name] will not fit into the [src.name] because they have a slime latched onto their head.")
					return
			var/mob/M = G:affecting//affecting//victim
			if(put_mob(M))
				qdel(G)
		src.updateUsrDialog()
		return
