...
/obj/structure/railing/attackby(var/obj/item/W, var/mob/user)
	...
	if(istype(W, /obj/item/grab) && get_dist(src,user)<2)
		...
		if(istype(G.victim, /mob/living/carbon/human))
			...
			if(G.force_danger())
				if(user.a_intent == I_HURT)
					visible_message("<span class='danger'>[G.attacker] slams [G.victim]'s face against \the [src]!</span>")
					playsound(loc, 'sound/effects/grillehit.ogg', 50, 1)
					var/blocked = G.victim.run_armor_check(BP_HEAD, "melee")
					if (prob(30 * blocked_mult(blocked)))
						G.victim.Weaken(5)
					G.victim.apply_damage(8, BRUTE, BP_HEAD, blocked)
				else
					if (get_turf(G.victim) == get_turf(src))
						G.victim.forceMove(get_step(src, src.dir))
					else
						G.victim.forceMove(get_turf(src))
					G.victim.Weaken(5)
					visible_message("<span class='danger'>[G.attacker] throws \the [G.victim] over \the [src].</span>")
			else
