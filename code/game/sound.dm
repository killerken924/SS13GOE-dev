...
/proc/playsound(var/atom/source, soundin, vol as num, vary, extrarange as num, falloff, var/is_global, var/frequency, var/is_ambiance = 0,skiprandfreq=0)
	...
	if(!is_global)
		...
		if(istype(src,/mob/living/))
			...
			if (ishuman(M))
				...
				else if(H.Get_Insanity()>=2&&H.Get_Insanity()<4)
