...
/obj/machinery/resleever/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/organ/internal/stack))
		...
		if(!check_occupant_allowed(grab.victim))
			return

		var/mob/M = grab.victim

		visible_message("[user] starts putting [grab.victim:name] into \the [src].", 3)

		if(do_after(user, 20, src))
			if(!M || !grab || !grab.victim) return
