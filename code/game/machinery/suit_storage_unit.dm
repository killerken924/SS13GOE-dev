...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/grab) )
		...
		if ( (occupant) || (helmet ) || (suit) || (boots) || (tank) || (mask)) //Unit needs to be absolutely empty
			...
			return
		visible_message("[user] starts putting [G.victim.name] into the Suit Storage Unit.", 3)
		if(do_after(user, 20, src))
			if(!G || !G.victim) return //derpcheck
			var/mob/M = G.victim
			if (M.client)
...
/obj/machinery/suit_cycler/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/grab))
		...
		if(contents.len > 0)
			to_chat(user, "<span class='danger'>There is no room inside the cycler for [G.victim.name].</span>")
			return

		visible_message("<span class='notice'>[user] starts putting [G.victim.name] into the suit cycler.</span>", 3)

		if(do_after(user, 20, src))
			if(!G || !G.victim) return
			var/mob/M = G.victim
			if (M.client)
				M.client.perspective = EYE_PERSPECTIVE
				M.client.eye = src
