		...
		for(var/gear_slot in H.species.hud.gear)
			...
			if(inventory_shown && hud_shown)
				switch(hud_data["slot"])
					...
					if(slot_underwear)
						if(H.wear_underwear) H.wear_underwear.screen_loc = hud_data["loc"]
					if(slot_bra)
						if(H.wear_bra) H.wear_bra.screen_loc = hud_data["loc"]
			else
				switch(hud_data["slot"])
					if(slot_head)
					...
					if(slot_underwear)
						if(H.wear_underwear) H.wear_underwear.screen_loc = null
					if(slot_bra)
						if(H.wear_bra) H.wear_bra.screen_loc = null
