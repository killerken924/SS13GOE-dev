...
/obj/item/organ/external/update_icon(var/regenerate = 0)
	...
	var/unique_body=owner.traits.Find(/datum/newtraits/unique_body)
	if(unique_body&&owner.gender==FEMALE)
		icon = 'icons/mob/female.dmi'
	if(force_icon)
		icon = force_icon
	else if (!dna)
	...
	/*if(owner.gender == FEMALE)//do boobs
		var/icon/breasticon=icon(icon,"breast[breast_size]")
		if(C.s_tone >= 0)
			breasticon.Blend(rgb(s_tone, s_tone, s_tone), ICON_ADD)
		else
			breasticon.Blend(rgb(-s_tone, -s_tone, -s_tone), ICON_SUBTRACT)
		mob_icon.Blend(breasticon, ICON_OVERLAY) 	*/
