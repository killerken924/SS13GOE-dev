...
/obj/item/organ/external/chest/update_icon(var/regenerate = 0)
	..(regenerate)
...
/obj/item/organ/external/groin
	...
	var/genital
	var/list/genitalcavity=list()
	var/genitalcarrysize

/obj/item/organ/external/groin/proc/CanPutInGenitals(obj/item/I )
	if(genital=="Vagina")//Bigger
		if(I.w_class<=genitalcarrysize&&!genitalcavity.len)
			return 1
		else
			return 0
	if(genital=="Penis")//Tiny only
		if(I.w_class<=genitalcarrysize&&!genitalcavity.len)
			return 1
		else
			return 0

/obj/item/organ/external/groin/New(var/mob/living/carbon/holder)
	..(holder)
	MakeGenitals()

/obj/item/organ/external/groin/proc/MakeGenitals()
	genital=(owner.gender==MALE ? "Penis" : "Vagina")
	if(genital=="Penis")
		genitalcarrysize=ITEM_SIZE_TINY
	if(genital=="Vagina")
		genitalcarrysize=pick(ITEM_SIZE_NORMAL)
