...
/mob/living/carbon/throw_item(atom/target)
	...
	if (istype(item, /obj/item/grab))
		...
		if(ismob(item))
			...
			if(ishuman(src))
				staminatake=M.mob_size/src.mob_size
				var/mob/living/carbon/human/H=src
				var/datum/realskills/strength=H.Skills.get_skill(/datum/realskills/strength)
				if(strength&&strength.points)
					staminatake=max(M.mob_size/src.mob_size-strength.points/20,0)
			Do_Stamina(staminatake)
			throw_range = round(M.throw_range * min((src.mob_size/M.mob_size)/3, 0.3))
