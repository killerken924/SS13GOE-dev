...
/mob/living/carbon/human/u_equip(obj/W as obj)
	...
	if (W == wear_suit)
		...
		update_inv_glasses()
	//
	else if (W == wear_underwear)
		wear_underwear = null
		update_inv_wear_underwear()
	else if (W == wear_bra)
		wear_bra = null
		update_inv_wear_bra()
	//
...
/mob/living/carbon/human/equip_to_slot(obj/item/W as obj, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		//
		if(slot_underwear)
			src.wear_underwear = W
			W.equipped(src, slot)
			update_inv_wear_underwear(redraw_mob)

		if(slot_bra)
			src.wear_bra = W
			W.equipped(src, slot)
			update_inv_wear_bra(redraw_mob)
		//
		else
...
/mob/living/carbon/human/get_equipped_item(var/slot)
	switch(slot)
		...
		if(slot_underwear)	return wear_underwear
		if(slot_bra)      	return wear_bra
	return ..()
...
/mob/living/carbon/human/get_equipped_items(var/include_carried = 0)
	...
	//
	if(wear_underwear) .+=wear_underwear
	if(wear_bra) .+=wear_bra
	//
	if(include_carried)
		if(l_store)    . += l_store
		if(r_store)    . += r_store
