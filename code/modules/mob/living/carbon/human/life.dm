...
#define HUMAN_MAX_OXYLOSS 10//1 //Defines how much oxyloss humans can get per tick. A tile with no air at all (such as space) applies this value, otherwise it's a percentage of it.
...
/mob/living/carbon/human/Life()
	...
	if(!Skills.skills.len||!Skills)
		Populate_Basic_Skills()
...
/mob/living/carbon/human/handle_chemicals_in_body()
	...
	// Trace chemicals
	//The stomach deals with this 'shit' now... hehe...get it?
	/*
...
/mob/living/carbon/human/handle_regular_status_updates()
	...
	if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if(paralysis || sleeping)
			...
			if(sleeping)
				...
				if (mind)
					if(client)
						if(selectedsleep)
							if(selectedsleep&&sleeping > 5)
								AdjustSleeping(-1)
						else
							AdjustSleeping(-1)
							/*switch(ep)
								if(EP_SAFE to max_ep)
									AdjustSleeping(-1*ep/2)
								if(EP_OKAY to EP_SAFE)
									AdjustSleeping(-1)
								if(EP_BAD  to EP_SAFE)
									AdjustSleeping(-0.5)
								if(EP_SERIOUS  to EP_BAD)
									AdjustSleeping(-0.2)*/
					//Are they SSD? If so we'll keep them asleep but work off some of that sleep var in case of stoxin or similar.
					if(sleeping > 3&&!client)
						AdjustSleeping(-1)
		...
		// hydration decrease
		if (hydration > 0)
			hydration = max (0, hydration - species.thirst_factor)

		handle_thirst_and_hunger()
