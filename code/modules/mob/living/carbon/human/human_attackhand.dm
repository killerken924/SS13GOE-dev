...
/mob/living/carbon/human/attack_hand(mob/living/carbon/M as mob)
	...
	switch(M.a_intent)
		...
		if(I_HURT)
			var/datum/realskills/attacker_endurance_skill=H.Skills.get_skill(/datum/realskills/endurance)
			var/dodge_stamina_amt=max(rand(0.5,1)-0.1*(attacker_endurance_skill.points),0)//so if you had 15, it would 15*0.1=1.5
			if(dodge_stamina_amt)
				H.Do_Stamina(dodge_stamina_amt)//Dodging takes stamina from the dodger
			...
			if(!affecting || affecting.is_stump())
				...
				return 1
			if(attempt_defence(M)&&!sleeping&&!stat==UNCONSCIOUS)
				return 1
			switch(src.a_intent)
				if(I_HELP)
			...
			if(strength_skill&&strength_skill.points)
				real_damage*=strength_skill.points/5  // 15*3= 45, 15*.2=3, so if you had max strength you would do 3 times the amount of damage, and if you had minimum strength you would do a 5th of damage.
			real_damage = max(1, real_damage)
			H.Do_Stamina(attack.stamina_drain)//
...
/mob/living/carbon/human/proc/break_all_grabs(mob/living/carbon/user)
	...
	if(istype(l_hand, /obj/item/grab))
		...
		if(lgrab.victim)
			visible_message("<span class='danger'>[user] has broken [src]'s grip on [lgrab.victim]!</span>")
			success = 1
		spawn(1)
			qdel(lgrab)
	if(istype(r_hand, /obj/item/grab))
		...
		if(rgrab.victim)
			visible_message("<span class='danger'>[user] has broken [src]'s grip on [rgrab.victim]!</span>")
			success = 1
		spawn(1)
			qdel(rgrab)
	return success
