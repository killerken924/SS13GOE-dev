			...
			if (!now_pushing)
				...
				if(ishuman(AM) && AM:grabbed_by)
					for(var/obj/item/grab/G in AM:grabbed_by)
						step(G:attacker, get_dir(G:attacker, AM))
						G.adjust_position()
				now_pushing = 0
			return
	return
...
/mob/living/proc/get_organ_target()
	...
	if ((t in list( BP_EYES, BP_MOUTH, BP_NECK )))
		t = BP_HEAD
						...
						if (prob(75))
							...
							if (istype(G, /obj/item/grab))
								for(var/mob/O in viewers(M, null))
									O.show_message(text("<span class='warning'>[] has been pulled from []'s grip by []</span>", G.victim, G.attacker, src), 1)
								//G = null
								qdel(G)
						else
							ok = 0
