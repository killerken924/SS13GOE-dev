...
/mob/living/carbon/human/handle_fall_effect(var/turf/landing)
	...
	..()
	var/damage = rand(5,60)
	if(prob(50))
		apply_damage(rand(7, damage), BRUTE, BP_L_LEG)
		apply_damage(rand(7, damage), BRUTE, BP_R_LEG)
		weakened = max(weakened,20)
		updatehealth()
		return
	if(prob(25))
		apply_damage(rand(5, damage), BRUTE, BP_HEAD)
		weakened = max(weakened,80)
		updatehealth()
		return
	else
